...
data_str="D:\matlab工具箱\代码\dataset.xlsx";  %读取数据的路径 	
...
% ====================== 手动特征数量设置（保留REF框架） ======================
manual_feature_num = 15;  % 手动设置需要选择的特征数量
index_name = data_biao1;
print_index_name = []; 

% 运行REF特征选择
opts = statset('Display','iter');
[REF_tf, history] = sequentialfs(@REF_regress, A_data1(:,1:end-1), A_data1(:,end), ...
                                'Direction','backward','Options',opts);

% 获取所有特征的重要性顺序（从最重要的开始）
feature_importance_order = [];
for i = 1:size(history.In,1)
    % 找出每次迭代中移除的特征
    if i == 1
        current_features = true(1, size(A_data1,2)-1);
    else
        current_features = history.In(i-1,:);
    end
    
    removed_feature = find(current_features & ~history.In(i,:));
    feature_importance_order = [removed_feature, feature_importance_order];
end

% 添加最后保留的特征（这些是最重要的）
final_features = find(history.In(end,:));
feature_importance_order = [final_features, feature_importance_order];

% 选择最重要的N个特征
feature_need_last = feature_importance_order(1:min(manual_feature_num, length(feature_importance_order)));
